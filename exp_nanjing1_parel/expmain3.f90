  ...
  call SYSTEM_CLOCK(iTimes1)
    ...
    Open (23, File='jac_main.dat', Status='unknown')
...
!        Write(6,*)'calculate Jacobi matrix ...'
  ...
  call SYSTEM_CLOCK(iTimes2)
         call cal_jacobi(rank, nproc_current, 1, rho_iter, hh_iter,jacobi1, nolayer, ntc, ns_index,&
         & point1set, point2set, point3set, point4set,htt,ns_real,t_st,t_ed,xr1, hr1,rt1, rr1,nturn1,nturn11)
  ...
  call SYSTEM_CLOCK(iTimes4)
    open (33, File='expmain3_record.txt', status='unknown')
   if(rank==0)then
...
360 ks = ks - 1
    ...
    subroutine cal_jacobi(rank, nproc_current, workPerProcess, m1,m2,jac,nlayer,ntc0,ns_id1,p11,p21,p31,&
    p41,htt1,ns_real1,t_st1,t_ed1,xr1,hr1,rt1,rr1,nturn1,nturn11)
    use mpi
    integer:: i,j,no,ntc0,ns_id1,ns_real1,nturn1,nturn11
    
    real *8 htt1
    Real *8 m1(nlayer), m2(nlayer), delta,jac_perRank(ntc0), jac_allRank(ntc0)
    ...
    do j=1,9!no
        ...
        call cal_diff(m1,m2,zeta,delta,j,jac_perRank,nlayer,ntc0,ns_id1,p11,p21,p31,p41,htt1,ns_real1,&
        & t_st1,t_ed1,xr1,hr1,rt1,rr1,nturn1,nturn11)
        


        call MPI_barrier(MPI_COMM_WORLD, ierr)
        ! call MPI_Reduce(jac_perRank,jac_allRank,ntc0,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
        call MPI_ALLReduce(jac_perRank,jac_allRank,ntc0,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
        
            do i=1,ntc0
                jac(i,j)=jac_allRank(i)
            end do
        ! end if
    end do
    ...
    Real *8 zeta1(1), del, rhoo(nlayer), hhh(nlayer), temp(ntc1),htt2
